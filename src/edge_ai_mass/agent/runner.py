...
from edge_ai_mass.agent.envelopes import Envelope
...
class EdgeDeviceAgent:
    ...
    def publish_event(
        ...
        *,
        ...
        if self.mqtt is not None and self.mqtt.is_started:
            return self.mqtt.publish_event(
                event_name,
                payload,
                correlation_id=correlation_id,
                request_id=request_id,
        ...
        if self.mqtt is not None:
            envelope = self.mqtt.publish_event(
                event_name,
                payload,
                correlation_id=correlation_id,
                request_id=request_id,
        ...
        for record in self.outbox.due_records():
            try:
                if record.kind == "telemetry":
                    ...
                    if self.mqtt is None or not self.mqtt.is_started:
